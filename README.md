...
**For example:**   
You can use the **fullattack** (*preattack*:`auxiliary/hashes/pack_maskgen`, *attack*:`attack/hashes/hashcat_masks`, *postattack*=`auxiliary/hashes/hashes_status`). First, it will generate appropiate masks with `auxiliary/hashes/pack_maskgen` auxiliary module and then it will perform an attack using `attack/hashes/hashcat_wordlist` attack module with the generated masks and finally it will run `auxiliary/hashes/hashes_status` auxiliary module, which report the hashes status.
